...
def find_classes(dir, test_mode):
...
def make_dataset(dir, class_to_idx, test_mode):
    spects = []
    ...
    if not test_mode:
        ...
        for root, _, fnames in sorted(os.walk(dir)):
            for fname in sorted(fnames, key=lambda x: int(x.partition('.')[0])):
                if is_audio_file(fname):
                    path = os.path.join(root, fname)
                    item = path
                    spects.append(item)
...
class GCommandLoader(data.Dataset):
    ...
    def __init__(self, root, transform=None, target_transform=None, window_size=.02,
        ...
        classes, class_to_idx = find_classes(root, test_mode)
        spects = make_dataset(root, class_to_idx, test_mode)
